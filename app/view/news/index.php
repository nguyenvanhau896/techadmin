...
<?php require_once '../app/component/nav.php'?>
    ...
    <button type="button" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md ms-8 flex flex-row justify-center text-center items-center h-[5rem]"> 
        <span class="text-[1.6rem]"> Thêm bài viết mới </span>
    ...
    <div class="text-[2.4rem] p-[2rem] font-bold text-slate-700"> Danh sách bài viết: </div>
    ...
    //  $query1 = "SELECT DISTINCT * FROM paper order by RAND() limit 3";
     ...
     forEach ($papers as $paperItem) {
        ...
        <a href="/techadmin/admin/index?id='.$paperItem['id'].'" class="d-flex">';
        echo '<img class="rounded-t-lg p-3 max-w-20" src="'.$paperItem['image'].'" anh san pham" />';
        echo '</a>';
        echo '<div class="px-4 pb-10">
                <a href="/techadmin/admin/paper/index?id='.$paperItem['id'].'">
        ...
        echo '<a href="/techadmin/admin/paper/index?id='.$paperItem['id'].'" class="absolute text-[1.2rem] bottom-2 left-[2rem] inline-flex items-center px-3 py-2  font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 w-[8.6rem] text-center flex flex-row justify-center">
                    Chỉnh sửa
        ...
        echo '<a href="/techadmin/admin/paper/index?id='.$paperItem['id'].'" class="absolute text-[1.2rem] bottom-2 right-[2rem] inline-flex items-center px-3 py-2  font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 w-[8.6rem] text-center flex flex-row justify-center">
                Xóa
